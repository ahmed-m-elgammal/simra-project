...
export function auditDecision(decision: any, personaIds: string[]): AuditFinding[] {
  ...
  for (const p of personaIds) {
    const vecs = decision.options.map((o: any) => o.persona_effects.find((e: any) => e.persona_id === p)?.delta ?? {});
    const keys = [...new Set(vecs.flatMap((v: any) => Object.keys(v)))];
    if (keys.length > 0 && vecs.every((v: any) => keys.every((k) => (v[k] ?? 0) === (vecs[0][k] ?? 0)))) {
      out.push({ code: "MEANINGLESS", message: `all options identical for ${p} in ${decision.id ?? "decision"}` });
    }
    for (let i = 0; i < vecs.length; i++) {
      const dom = vecs.every((v: any, j: number) => j === i || keys.every((k) => (vecs[i][k] ?? 0) >= (v[k] ?? 0)));
      const strict = vecs.some((v: any, j: number) => j !== i && keys.some((k) => (vecs[i][k] ?? 0) > (v[k] ?? 0)));
      if (keys.length > 0 && dom && strict) {
