...
Global flags on every command: `--workdir` (required), `--format json|human` (default human on TTY, json when piped). Exit mapping: `CliError` code MISSING_INPUT → 3, VALIDATION → 2, GATE_OPEN/ABORTED → 4, CONFLICT → 5; `--format json` prints `{ ok:false, code, message, details }` on stdout.
...
Locked as built: leaf commands are also named-exported (`submit`, `approve`, …) because citty's `runCommand` on a parent does not forward leaf results — tests and agents drive leaves directly. `Detail.code` is a plain string (stage codes like `SEGMENT_*`/`SCHEMA` live outside the builder's union). `approve` acts on invocation (explicit by construction); clack only prompts for a missing `--note` in `request-changes` on a TTY. Chapter submit runs the auditor per decision: `MEANINGLESS` blocks, `DOMINANT` lands in `warnings`.
...
`adapters.ts`: `StorageAdapter { uploadImmutable(name, bytes, contentType): Promise<url>; uploads(): string[] }`, `DbAdapter { getBook(book_id): Promise<BookRow|null>; setPublished(book_id, row): Promise<void> }` where `BookRow = { version, bundle_url, sha }`, `FakeStorage` (in-memory, DUPLICATE error on re-upload of the same name), `FakeDb` (in-memory rows), `SupabaseStorage`/`SupabaseDb` (thin `supabase-js` wrappers, service key from env only, manual-tested once against a scratch project — record project ref + test file name in the commit message, never commit keys). Requires `bundle_sha` on `books` (added to Plan 03 migration).

`publish --dry-run`: validates, assembles, prints what would flip (version N→N+1, sha, bytes, target URL shape; push cohort resolves server-side in the publish-hook) and changes nothing. `publish`: validate green → assemble → anchor on the dist file's version (NOT `next_version`: build already bumped it) → byte-compare dist vs fresh compile (stale → VALIDATION) → sha-identical → skip without uploading → else require dist version == row version + 1 → upload 3 files (json/br/gz, immutable, no upsert) → pointer swap → print `{ ok:true, version, url, sha }`. Duplicate-race: on DUPLICATE upload error, re-read the row — identical sha returns existing, different content throws CONFLICT (exit 5, re-pull and rebuild). Order locked: upload first, swap second (spec rule). Minibook seeding shared via `cli/fixtures/minibook/seed-workdir.ts` (no duplicated setup across test files).
